...
if __name__ == '__main__':
    ...
    for i in range(len(years)):
        ...
        tempPhotoPath = os.path.join(DATA_FOLDER, "temp_negi_{}.tif".format(iYear))
        ...
        outFileName = r'{data_folder}\negi_{year}.tif'\
            .format(data_folder=DATA_FOLDER, year=iYear)
        # cmdStr = r'gdal_calc --calc "((2*B-A-C)/(2*B+A+C))*D" --format GTiff --type Float32 --overwrite ' \
        ...
        cmdStr = r'gdal_calc --calc "((2*B-A-C)/(2*B+A+C))" --format GTiff --type Float32 --overwrite ' \
                 r'-A {src_file} --A_band 1 ' \
                 r'-B {src_file} --B_band 2 ' \
                 r'-C {src_file} --C_band 3 ' \
                 r'--outfile={out_name}'\
